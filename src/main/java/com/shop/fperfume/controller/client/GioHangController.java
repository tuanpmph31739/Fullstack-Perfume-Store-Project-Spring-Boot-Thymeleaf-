...
import com.shop.fperfume.service.client.GioHangClientService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.ResponseEntity; // SỬA 1: Import
import org.springframework.security.core.annotation.AuthenticationPrincipal;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.*;
// SỬA 2: Xóa RedirectAttributes vì không cần dùng cho AJAX
// import org.springframework.web.servlet.mvc.support.RedirectAttributes;
...
public class GioHangController {
    ...
    /**
     * HIỂN THỊ trang giỏ hàng (Không đổi)
     */
    @GetMapping
    public String viewCart(Model model,
                           @AuthenticationPrincipal CustomUserDetails userDetails) {
    ...
    /**
     * THÊM sản phẩm vào giỏ (AJAX) (Không đổi)
     */
    @PostMapping("/add")
    @ResponseBody
    public ResponseEntity<?> addItemToCart(
    ...
    /**
     * CẬP NHẬT số lượng (AJAX) (Không đổi)
     */
    @PostMapping("/update")
    @ResponseBody
    public ResponseEntity<?> updateItemQuantity(
    ...
    public ResponseEntity<?> applyVoucher(@RequestParam("maGiamGia") String maGiamGia,
        ...
        try {
            gioHangClientService.applyVoucher(khachHang, maGiamGia);
            // Trả về 200 OK. JavaScript sẽ thấy (res.ok) và tự reload trang.
            return ResponseEntity.ok().build();

        } catch (Exception e) {
            // SỬA 4: Trả về lỗi 400 (Bad Request)
            // JavaScript sẽ thấy (!res.ok) và chạy vào khối 'else' để hiển thị SweetAlert
            return ResponseEntity.badRequest().body(Map.of("success", false, "message", e.getMessage()));
        }
    }

    /**
     * SỬA 5: Tương tự, trả về ResponseEntity (JSON)
     */
    @PostMapping("/remove-voucher")
    @ResponseBody // Đảm bảo trả về JSON
    ...
    /* =====================================================================
     * HÀM HELPER TÍNH TOÁN VÀ TẠO RESPONSE (Không đổi)
     * ===================================================================== */
